...
import com.daoke360.dao.SparkTaskDao
...
object BAreaTop3ProductTask {
  ...
  def processArgs(args: Array[String], sparkConf: SparkConf) = {
    if (args.length == 2 && args(0).equals(GlobalContants.TASK_PARAMS_FLAG) && Utils.validateNumber(args(1))) {
      ...
      throw new SparkException(s"输入的参数有误...")
    }
  }
  ...
  def registAreaProductClickTotalCountInfoTmp(sparkSession: SparkSession) = {
    ...
    sparkSession.sql(
      """
        |select province,gid,sum(click_count) click_count,city_concat(city) from area_product_click_count_tmp
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (sparkTask == null)
      throw new SparkException(s"从数据库获取任务信息失败...")

    val taskParamObject = JSON.parseObject(sparkTask.task_param)
    //初始化扫描器
    val scan = initScan(taskParamObject)
    //将scan转换成字符串
    val ptotoScan = ProtobufUtil.toScan(scan)
    val base64Scan = Base64.encodeBytes(ptotoScan.toByteArray)
    ...
    //去读取hbase中的数据
    val eventLogRDD = sparkSession.sparkContext.newAPIHadoopRDD(jobConf, classOf[TableInputFormat], classOf[ImmutableBytesWritable], classOf[Result])
      .map(_._2).map(result => {
      val eventName = Bytes.toString(result.getValue(EventLogContants.HBASE_EVENT_LOG_TABLE_FAMILY.getBytes(),
        EventLogContants.LOG_COLUMN_NAME_EVENT_NAME.getBytes()))
      val province = Bytes.toString(result.getValue(EventLogContants.HBASE_EVENT_LOG_TABLE_FAMILY.getBytes(),
        EventLogContants.LOG_COLUMN_NAME_PROVINCE.getBytes()))
      val city = Bytes.toString(result.getValue(EventLogContants.HBASE_EVENT_LOG_TABLE_FAMILY.getBytes(),
        EventLogContants.LOG_COLUMN_NAME_CITY.getBytes()))
      val gid = Bytes.toString(result.getValue(EventLogContants.HBASE_EVENT_LOG_TABLE_FAMILY.getBytes(),
        EventLogContants.LOG_COLUMN_NAME_GOODS_ID.getBytes()))
